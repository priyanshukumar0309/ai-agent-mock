...
export function GeminiNode({ status, isExpanded }: Props) {
    ...
    >
      <div className="flex items-center gap-1.5">
        <div className={`relative ${isExpanded ? 'w-10 h-10' : 'w-7 h-7'} rounded-lg flex items-center justify-center transition-all duration-300 ${
          isActive ? 'bg-blue-100' : isComplete ? 'bg-emerald-100' : 'bg-snow-100'
        }`}>
          <img
          ...
          {isActive && (
            <motion.div
              className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-volvo-blue border-[1.5px] border-white"
        ...
        <div>
          ...
          <p className={`${isExpanded ? 'text-[9px]' : 'text-[7px]'} text-snow-400 leading-tight`}>Google</p>
        </div>
      </div>
    </motion.div>
  );
